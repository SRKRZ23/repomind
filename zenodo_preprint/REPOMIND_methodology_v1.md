...
| Cursor Business equivalent (1000 dev × $40/month × 12 months, Cursor Business tier as published mid-2026) | $480,000/year |
| Break-even (owned MI300X vs Cursor Business, team of 100, illustrative) | **3-6 months** |
...
¹ Wall Street Journal coverage of JPMorgan Chase internal restriction on ChatGPT, reported February 2023. Public news archives index the article under variants of the title "JPMorgan Restricts Employees From Using ChatGPT."

² Wall Street Journal coverage of Apple internal restriction on ChatGPT and GitHub Copilot, reported May 2023. Public news archives index the article under variants of the title "Apple Restricts Employees From Using ChatGPT."

³ AMD Developer Resources technical article, "Day-0 Support for Qwen3-Coder-Next on AMD Instinct GPUs," February 2026. The author accessed this article at:
...
⁷ AMD ROCm performance documentation and vLLM-on-ROCm coverage at https://rocm.docs.amd.com/ ; specific MI300X attention-backend benchmarks are referenced indirectly via the AMD ROCm blog. The author did not verify every individual blog URL in this preprint and instead points to the top-level documentation root for current references.

⁸ Wang et al., "OpenDevin: An Open Platform for AI Software Developers as Generalist Agents," 2024. Project: https://github.com/OpenDevin/OpenDevin

⁹ Aider — AI pair programming in your terminal. Project: https://github.com/Aider-AI/aider

¹⁰ Yang et al., "SWE-agent: Agent-Computer Interfaces Enable Automated Software Engineering," NeurIPS 2024. Project: https://github.com/SWE-agent/SWE-agent

¹¹ AMD Developer Community thread (May 2026) containing the public technical questions posted by Hakob Arzumanyan that motivated Phase 2 of this work. The thread URL is on the `devcommunity.amd.com/` domain; the exact thread identifier is captured in the REPOMIND repository at `competitions/repomind/slides/HAKOB_FOLLOWUP_REPLY.md` for readers who wish to trace the public exchange.
...
**Citation:** Razikov, S. (2026). REPOMIND: Reproducing 256K-context Repository-Scale Code Understanding on a Single AMD MI300X with FP8 KV Cache. Zenodo. [DOI pending]
